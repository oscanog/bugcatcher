...
function bc_v1_dashboard_summary_get(mysqli $conn, array $params): void
{
    ...
    if ($isAllScope) {
        ...
        if ($orgIds) {
            ...
            $stmt = $conn->prepare("
                ...
                FROM projects
                WHERE status = 'active' AND org_id IN ({$placeholders})
            ");
            bc_v1_stmt_bind($stmt, str_repeat('i', count($orgIds)), $orgIds);
            $stmt->execute();
            ...
            $stmt = $conn->prepare("
                ...
                WHERE status IN ('open', 'in_progress', 'blocked', 'failed') AND org_id IN ({$placeholders})
            ");
            bc_v1_stmt_bind($stmt, str_repeat('i', count($orgIds)), $orgIds);
            $stmt->execute();
            $row = $stmt->get_result()->fetch_assoc();
            ...
            $summary['open_issues'] = bc_v1_issue_count_for_org_ids($conn, $orgIds, 'open');
            $summary['closed_issues'] = bc_v1_issue_count_for_org_ids($conn, $orgIds, 'closed');

            $rangeParams = array_merge($orgIds, [$days[0], $days[count($days) - 1]]);
            $rangeTypes = str_repeat('i', count($orgIds)) . 'ss';

            $issueTrendStmt = $conn->prepare("
                SELECT DATE(i.created_at) AS day_key, COUNT(*) AS total
                FROM issues i
                WHERE i.org_id IN ({$placeholders}) AND DATE(i.created_at) BETWEEN ? AND ?
                GROUP BY DATE(i.created_at)
                ORDER BY DATE(i.created_at) ASC
            ");
            bc_v1_stmt_bind($issueTrendStmt, $rangeTypes, $rangeParams);
            ...
            $projectTrendStmt = $conn->prepare("
                ...
                FROM projects
                WHERE org_id IN ({$placeholders}) AND DATE(created_at) BETWEEN ? AND ?
                GROUP BY DATE(created_at)
                ORDER BY DATE(created_at) ASC
            ");
            bc_v1_stmt_bind($projectTrendStmt, $rangeTypes, $rangeParams);
            ...
            $checklistTrendStmt = $conn->prepare("
                ...
                ORDER BY DATE(created_at) ASC
            ");
            bc_v1_stmt_bind($checklistTrendStmt, $rangeTypes, $rangeParams);
            ...
            $recentIssuesStmt = $conn->prepare("
                ...
                LIMIT 5
            ");
            bc_v1_stmt_bind($recentIssuesStmt, str_repeat('i', count($orgIds)), $orgIds);
    ...
    $recentIssuesStmt = $conn->prepare("
        ...
        LIMIT 5
    ");
    $recentIssuesStmt->bind_param('si', $org['org_name'], $org['org_id']);
    $recentIssuesStmt->execute();
    $recentIssues = $recentIssuesStmt->get_result()->fetch_all(MYSQLI_ASSOC);
    $recentIssuesStmt->close();
